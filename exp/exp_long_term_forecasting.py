...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        with open("telltailtrain.txt", "w", encoding="utf-8") as telltail:
            train_data, train_loader = self._get_data(flag='train')
            vali_data, vali_loader = self._get_data(flag='val')
            test_data, test_loader = self._get_data(flag='test')

            telltail.write(f"timestamp_start: {datetime.datetime.now()}\n")
            telltail.write(f"train_data: {len(train_data)}\n")
            telltail.write(f"vali_data: {len(vali_data)}\n")
            ...
            for epoch in range(self.args.train_epochs):
                ...
                telltail.write(f"Epoch: {epoch + 1:.7f}, Steps: {train_steps:.7f} | Train Loss: {train_loss:.7f} Vali Loss: {vali_loss:.7f} Test Loss: {test_loss:.7f}\n")
                ...
                if early_stopping.early_stop:
                    ...
                    telltail.write(f"Early stopping at epoch: {epoch + 1}\n")   
                    break

                adjust_learning_rate(model_optim, epoch + 1, self.args)

            telltail.write(f"timestamp_end: {datetime.datetime.now()}\n")
